...
-- =============================================
CREATE PROCEDURE [dbo].[Scheduler_Admin_test] 
AS
BEGIN
	...
	insert into @TruckStatus (OrganisationId,OrganisationName,OrgShortName)
		...
		WHILE(@Counter IS NOT NULL
			...
			-- commented on 13-Sep-2021 as suggested by MAdhav for changing 5 days to 120 hrs
				...
				--SELECT @currenttime  
				IF convert(varchar(26),@currenttimeslot,108) not between '09:00:00' AND '20:59:59'
				BEGIN 
			...
			select 0,'Total','Total',' ',sum(CheckInCount), sum(CheckOutCount), sum(TotalTrucks),sum(FiveDaysParkedTrucks) ,sum(TodayTrucks), 
